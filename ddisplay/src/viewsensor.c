...
gcc -g  viewsensor.c SDL2init.c -o viewsensorX86 -lSDL2 -lSDL2_image -lSDL2_ttf -lrt 

for ARM BBB compile with (note -D BBB)
export PATH=$PATH:$HOME/bbb2018/buildroot/output/host/bin ## for compiler
arm-linux-gnueabihf-gcc -g -D BBB -o viewsensorARM viewsensor.c SDL2init.c  -L /home/peter/bbb2018/buildroot/output/staging/usr/lib/ -lSDL2 -lSDL2_image -lSDL2_ttf -lrt
...
arm-linux-gcc -g -o viewsensorARM viewsensorARM.c SDL2init.c ~/Documents/daemons2016/pmtdiefaced/src/dietable.c --sysroot=/home/peter/igep2015/pmtstaging/   -lSDL2 -lSDL2_image -lSDL2_ttf  $(pkg-config --libs --cflags sdl2) $(pkg-config --libs --cflags SDL2_image)  $(pkg-config --libs --cflags SDL2_ttf)
...
{ 
	...
	SDL_SetColorKey( happy, SDL_TRUE,
						 SDL_MapRGB( happy->format, 0xFF, 0xFF, 0xFF ) );
	...
	while( !quit )
	{
		...
		sprintf (text, "%d/%d/%d %d:%d:%d",  tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		...
		sprintf ( text,  "%d:%d:%d%c", gpsnow->longitude/10000, 
				gpsnow->longitude%10000/100,
				gpsnow->longitude%100, gpsnow->longitudeEW );
		...
		sprintf ( text,  "%d:%d:%d%c", gpsnow->latitude/10000, 
				gpsnow->latitude%10000/100,
				gpsnow->latitude%100, gpsnow->latitudeNS );
