 ...
 *  The WMM source code is in the public domain and not licensed or under
 ...
 As required by 17 U.S.C. 403,
 *	third parties producing copyrighted works consisting predominantly of
 the material produced by *	U.S. government agencies must provide notice
 with such work(s) identifying the U.S. Government material *	incorporated and
...
int MAG_Grid(MAGtype_CoordGeodetic minimum, MAGtype_CoordGeodetic maximum,
             double cord_step_size, double altitude_step_size, double time_step,
             MAGtype_MagneticModel *MagneticModel, MAGtype_Geoid *Geoid,
             MAGtype_Ellipsoid Ellip, MAGtype_Date StartDate,
             MAGtype_Date EndDate, int ElementOption, int UncertaintyOption,
             int PrintOption, char *OutputFile)

/*This function calls WMM subroutines to generate a grid as defined by the user.
The function may be used to generate a grid of magnetic field elements, time
series or a profile. The selected geomagnetic element is either printed to the
file GridResults.txt or to the screen depending on user option.

INPUT: minimum :Data structure with the following elements (minimum limits of
the grid) double lambda; (longitude) double phi; ( geodetic latitude) double
HeightAboveEllipsoid; (height above the ellipsoid (HaE) ) double
HeightAboveGeoid;(height above the Geoid ) maximum : same as the above (maximum
limist of the grid) step_size  : double  : spatial step size, in decimal degrees
...
) EndDate :	Same as the above; MagneticModel :	 data structure with the
following elements double EditionDate; double epoch;       Base time of
...
vector will be needed by program Geoid :  data structure with the following
elements Pointer to data structure Geoid with the following elements int
NumbGeoidCols ;   ( 360 degrees of longitude at 15 minute spacing ) int
NumbGeoidRows ;   ( 180 degrees of latitude  at 15 minute spacing ) int
NumbHeaderItems ;    ( min, max lat, min, max long, lat, long spacing ) int
ScaleFactor;    ( 4 grid cells per degree at 15 minute spacing  ) float
*GeoidHeightBuffer;   (Pointer to the memory to store the Geoid elevation data )
                        int NumbGeoidElevs;    (number of points in the gridded
file ) int  Geoid_Initialized ;  ( indicates successful initialization ) Ellip
data  structure with the following elements double a; semi-major axis of the
ellipsoid double b; semi-minor axis of the ellipsoid double fla;  flattening
 ...
 *        UncertaintyOption: int: 1-Append uncertainties.  Otherwise do not
append uncertainties. PrintOption : int : 1 Print to File, Otherwise, print to
screen

   OUTPUT: none (prints the output to a file )

   CALLS : MAG_AllocateModelMemory To allocate memory for model coefficients
      MAG_TimelyModifyMagneticModel This modifies the Magnetic coefficients to
the correct date. MAG_ConvertGeoidToEllipsoidHeight (&CoordGeodetic, &Geoid);
Convert height above msl to height above WGS-84 ellipsoid
                  MAG_GeodeticToSpherical Convert from geodeitic to Spherical
Equations: 7-8, WMM Technical report MAG_ComputeSphericalHarmonicVariables
Compute Spherical Harmonic variables MAG_AssociatedLegendreFunction Compute ALF
Equations 5-6, WMM Technical report MAG_Summation Accumulate the spherical
harmonic coefficients Equations 10:12 , WMM Technical report
                  MAG_RotateMagneticVector Map the computed Magnetic fields to
Geodeitic coordinates Equation 16 , WMM Technical report
                  MAG_CalculateGeoMagneticElements Calculate the geoMagnetic
elements, Equation 18 , WMM Technical report
 ...
 */
{
  ...
  if (fabs(cord_step_size) < 1.0e-10)
    cord_step_size =
        99999.0; /*checks to make sure that the step_size is not too small*/
       ...
       minimum.HeightAboveGeoid += altitude_step_size) /* Altitude loop*/
  {
    ...
    for (minimum.phi = b; minimum.phi <= maximum.phi;
         minimum.phi += cord_step_size) /*Latitude loop*/
    {

      for (minimum.lambda = c; minimum.lambda <= maximum.lambda;
           minimum.lambda += cord_step_size) /*Longitude loop*/
      {
        if (Geoid->UseGeoid == 1)
          MAG_ConvertGeoidToEllipsoidHeight(
              &minimum, Geoid); /* This converts the height above mean sea level
                                   to height above the WGS-84 ellipsoid */
        else
          minimum.HeightAboveEllipsoid = minimum.HeightAboveGeoid;
        MAG_GeodeticToSpherical(Ellip, minimum, &CoordSpherical);
        MAG_ComputeSphericalHarmonicVariables(
            Ellip, CoordSpherical, MagneticModel->nMax,
            SphVariables); /* Compute Spherical Harmonic variables  */
        MAG_AssociatedLegendreFunction(
            CoordSpherical, MagneticModel->nMax,
            LegendreFunction); /* Compute ALF  Equations 5-6, WMM Technical
                                  report*/

        for (StartDate.DecimalYear = d;
             StartDate.DecimalYear <= EndDate.DecimalYear;
             StartDate.DecimalYear += time_step) /*Year loop*/
        {

          MAG_TimelyModifyMagneticModel(
              StartDate, MagneticModel,
              TimedMagneticModel); /*This modifies the Magnetic coefficients to
                                      the correct date. */
          MAG_Summation(LegendreFunction, TimedMagneticModel, *SphVariables,
                        CoordSpherical,
                        &MagneticResultsSph); /* Accumulate the spherical
                                                 harmonic coefficients Equations
                                                 10:12 , WMM Technical report*/
          MAG_SecVarSummation(
              LegendreFunction, TimedMagneticModel, *SphVariables,
              CoordSpherical,
              &MagneticResultsSphVar); /*Sum the Secular Variation Coefficients,
                                          Equations 13:15 , WMM Technical report
                                        */
          MAG_RotateMagneticVector(
              CoordSpherical, minimum, MagneticResultsSph,
              &MagneticResultsGeo); /* Map the computed Magnetic fields to
                                       Geodetic coordinates Equation 16 , WMM
                                       Technical report */
          MAG_RotateMagneticVector(
              CoordSpherical, minimum, MagneticResultsSphVar,
              &MagneticResultsGeoVar); /* Map the secular variation field
                                          components to Geodetic coordinates,
                                          Equation 17 , WMM Technical report*/
          MAG_CalculateGeoMagneticElements(
              &MagneticResultsGeo,
              &GeoMagneticElements); /* Calculate the Geomagnetic elements,
                                        Equation 18 , WMM Technical report */
          MAG_CalculateGridVariation(minimum, &GeoMagneticElements);
          MAG_CalculateSecularVariationElements(
              MagneticResultsGeoVar,
              &GeoMagneticElements); /*Calculate the secular variation of each
                                        of the Geomagnetic elements, Equation
                                        19, WMM Technical report*/
          ...
          case 1:
            PrintElement = GeoMagneticElements
                               .Decl; /*1. Angle between the magnetic field
          ...
          case 2:
            PrintElement =
                GeoMagneticElements
                    .Incl; /*2. Angle between the magnetic field vector and the
          ...
          case 4:
            PrintElement =
                GeoMagneticElements.H; /*4. Horizontal Magnetic Field Strength*/
            ErrorElement = Errors.H;
            break;
          case 5:
            PrintElement =
                GeoMagneticElements
                    .X; /*5. Northern component of the magnetic field vector*/
            ErrorElement = Errors.X;
            break;
          case 6:
            PrintElement =
                GeoMagneticElements
                    .Y; /*6. Eastern component of the magnetic field vector*/
            ErrorElement = Errors.Y;
            break;
          case 7:
            PrintElement =
                GeoMagneticElements
                    .Z; /*7. Downward component of the magnetic field vector*/
          ...
          case 8:
            ...
            break;
          case 9:
            PrintElement = GeoMagneticElements.Decldot *
                           60; /*9. Yearly Rate of change in declination*/
            UncertaintyOption = 0;
            break;
          case 10:
            PrintElement = GeoMagneticElements.Incldot *
                           60; /*10. Yearly Rate of change in inclination*/
            UncertaintyOption = 0;
            break;
          case 11:
            PrintElement =
                GeoMagneticElements.Fdot; /*11. Yearly rate of change in
                                             Magnetic field strength*/
            UncertaintyOption = 0;
            break;
          case 12:
            PrintElement =
                GeoMagneticElements.Hdot; /*12. Yearly rate of change in
                                             horizontal field strength*/
            UncertaintyOption = 0;
            break;
          case 13:
            PrintElement =
                GeoMagneticElements.Xdot; /*13. Yearly rate of change in the
                                             northern component*/
            UncertaintyOption = 0;
            break;
          case 14:
            PrintElement =
                GeoMagneticElements.Ydot; /*14. Yearly rate of change in the
                                             eastern component*/
            UncertaintyOption = 0;
            break;
          case 15:
            PrintElement =
                GeoMagneticElements.Zdot; /*15. Yearly rate of change in the
                                             downward component*/
            UncertaintyOption = 0;
            break;
          case 16:
          ...
          if (Geoid->UseGeoid == 1) {
            if (PrintOption == 1)
              fprintf(fileout, "%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi,
                      minimum.lambda, minimum.HeightAboveGeoid,
                      StartDate.DecimalYear, PrintElement);
            else
              printf("%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi,
                     minimum.lambda, minimum.HeightAboveGeoid,
                     StartDate.DecimalYear, PrintElement);
          } else {
            if (PrintOption == 1)
              fprintf(fileout, "%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi,
                      minimum.lambda, minimum.HeightAboveEllipsoid,
                      StartDate.DecimalYear, PrintElement);
            else
              printf("%5.2f %6.2f %8.4f %7.2f %10.2f", minimum.phi,
                     minimum.lambda, minimum.HeightAboveEllipsoid,
          ...
          /**Below can be used for XYZ Printing format (longitude latitude
           * output_data) fprintf(fileout, "%5.2f %6.2f %10.4f\n",
 ...
 */
{
  ...
  case 22:
    ...
    printf("Please download this file from "
...
char MAG_GeomagIntroduction_EMM(MAGtype_MagneticModel *MagneticModel,
  ...
  printf("\n Earth's main Magnetic field and crustal variation for a given "
         "point/area.");
  while (ans != 'c' && ans != 'C') {
    ...
    if ((ans == 'h') || (ans == 'H')) {
      ...
      printf("\n The Enhanced Magnetic Model (EMM) for %d",
             (int)MagneticModel->epoch);
      printf(
      ...
      printf("\n (Decl) which is sometimes called MagneticVAR, Inclination "
             "(Incl), Total");
      printf(
      ...
      printf("\n\n\n It is very important to note that a degree and  order 720 "
             "model,");
      printf(
          "\n such as EMM, describes the long  wavelength spatial Magnetic ");
      printf("\n fluctuations due to  Earth's core.  Also included in the EMM "
             "series");
      printf("\n models are intermediate and short wavelength spatial "
             "fluctuations ");
      printf("\n that originate in Earth's mantle and crust. Not included in");
      printf("\n the model are temporal fluctuations of Magnetospheric and "
             "ionospheric");
      printf("\n origin. On the days during and immediately following Magnetic "
             "storms,");
      printf("\n temporal fluctuations can cause substantial deviations of the "
             "Geomagnetic");
      printf("\n field  from model  values.  If the required  declination "
             "accuracy  is");
      printf("\n more stringent than the EMM  series of models provide, the "
             "user is");
      printf("\n advised to request special (regional or local) surveys be "
             "performed");
      printf(
          "\n and models prepared. Please make requests of this nature to the");
      printf("\n National Geospatial-Intelligence Agency (NGA) at the address "
             "below.");
...
char MAG_GeomagIntroduction_WMM(MAGtype_MagneticModel *MagneticModel,
                                char *VersionDate)
/*Prints the introduction to the Geomagnetic program.  It needs the Magnetic
 model for the epoch.

 * INPUT  MagneticModel		: MAGtype_MagneticModel With Model epoch
 (input) OUTPUT ans   (char)  user selection CALLS : none
 */
{
  char help = 'h';
  char ans;
  ...
  while (help != 'c' && help != 'C') {
    ...
    if ((help == 'h') || (help == 'H')) {
      ...
      printf("\n The World Magnetic Model (WMM) for %d",
             (int)MagneticModel->epoch);
      ...
      printf("\n is valid for 5 years after the base epoch. Uncertainties for "
             "the");
      printf("\n WMM are one standard deviation uncertainties averaged over "
             "the globe.");
      printf("\n We represent the uncertainty as constant values in Incl, F, "
             "H, X,");
      printf("\n Y, and Z. Uncertainty in Declination varies depending on the "
             "strength");
      printf("\n of the horizontal field.  For more information see the WMM "
             "Technical");
      printf("\n Report.");

      printf("\n\n\n It is very important to note that a  degree and  order 12 "
             "model,");
      printf("\n such as WMM, describes only the long  wavelength spatial "
             "Magnetic ");
      printf("\n fluctuations due to  Earth's core.  Not included in the WMM "
             "series");
      printf("\n models are intermediate and short wavelength spatial "
             "fluctuations ");
      printf("\n that originate in Earth's mantle and crust. Consequently, "
             "isolated");
      printf("\n angular errors at various  positions on the surface "
             "(primarily over");
      printf("\n land, along continental margins and  over oceanic sea-mounts, "
             "ridges and");
      printf("\n trenches) of several degrees may be expected.  Also not "
             "included in");
      printf("\n the model are temporal fluctuations of Magnetospheric and "
             "ionospheric");
      printf("\n origin. On the days during and immediately following Magnetic "
      ...
      printf("\n field  from model  values.  If the required  declination "
             "accuracy  is");
      printf("\n more stringent than the WMM  series of models provide, the "
             "user is");
      printf("\n advised to request special (regional or local) surveys be "
             "performed");
      printf("\n and models prepared. The World Magnetic Model is a joint "
             "product of");
      printf("\n the United States’ National Geospatial-Intelligence Agency "
             "(NGA) and");
      printf("\n the United Kingdom’s Defence Geographic Centre (DGC). The WMM "
             "was");
      printf("\n developed jointly by the National Geophysical Data Center "
             "(NGDC, Boulder");
      printf("\n CO, USA) and the British Geological Survey (BGS, Edinburgh, "
             "Scotland). ");
...
degrees a_step_size : double pointer :  double altitude step size (km) step_time
: double pointer : time step size (decimal years) StartDate : pointer to data
structure with the following elements updates double DecimalYear;     ( decimal
years ) EndDate :	Same as the above CALLS : none
 ...
 */
{
  ...
  if (Geoid->UseGeoid == 1)
    ...
    fprintf(fileout,
            "Minimum Altitude above MSL: %f\tMaximum Altitude above WGS-84 "
            "Ellipsoid: %f\tStep Size: %f\n",
...
coordinates and Date as objects. Returns 0 when the user wants to exit and 1 if
the user enters valid input data. INPUT :  MagneticModel  : Data structure with
...
following elements updated int	Year; (If user directly enters decimal year this
field is not populated) int	Month;(If user directly enters decimal year this
field is not populated) int	Day; (If user directly enters decimal year this
field is not populated) double DecimalYear;      decimal years
 ...
 */
{
  ...
  fgets(buffer, 40, stdin);
  for (i = 0, done = 0, j = 0; i <= 40 && !done;
       i++) /*This for loop determines how the user is trying to enter their
               data, and makes sure that it is copied into the correct
               location*/
  {
    if (buffer[i] ==
        '.') /*This detects if there is a '.' somewhere in the string, if there
                is the program tries to interpret the input as a double, and
                copies it to the longitude*/
    {
      j = sscanf(buffer, "%lf", &CoordGeodetic->lambda);
      if (j == 1)
        done = 1; /*This control ends the loop*/
      else
        done = -1; /*This copies an end string into the buffer so that the user
                      is sent to the Re-enter input message*/
    }
    if (buffer[i] == ',') /*This detects if there is a ',' somewhere in the
                             string, if there is the program tries to interpret
                             the input as Degrees, Minutes, Seconds.*/
    {
      if (MAG_ValidateDMSstringlong(buffer, Error_Message)) {
        MAG_DMSstringToDegree(
            buffer,
            &CoordGeodetic->lambda); /*The program uses this to convert the
                                        string into a decimal longitude.*/
        done = 1;
      } else
        done = -1;
    }
    ...
    if (buffer[i] == '\0' ||
            ...
            -1) /*If the program reaches the end of the string before finding a
                   "." or a "," or if its been sent by an error it does this*/
    {
      MAG_ValidateDMSstringlong(
          buffer, Error_Message); /*The program attempts to determine if all the
                                     characters in the string are legal, and
                                     then tries to interpret the string as a
                                     simple degree entry, like "0", or "67"*/
      if (MAG_ValidateDMSstringlong(buffer, Error_Message) && done != -1) {
        sscanf(buffer, "%lf", &CoordGeodetic->lambda);
        done = 1;
      } else /*The string is neither DMS, a decimal degree, or a simple degree
                input, or has some error*/
      {
        printf("%s", Error_Message);
        strcpy(buffer, ""); /*Clear the input*/
        printf("\nError encountered, please re-enter as '(-)DDD,MM,SS' or in "
               "Decimal Degrees DD.ddd:\n"); /*Request new input*/
        fgets(buffer, 40, stdin);
        i = -1; /*Restart the loop, at the end of this loop i will be
                   incremented to 0, effectively restarting the loop*/
        done = 0;
      }
    }
  }
  ...
  while (!done) {
    ...
    j = 0;
    if (buffer[0] == 'e' ||
        buffer[0] == 'E') /* User entered height above WGS-84 ellipsoid, copy it
    ...
    if (CoordGeodetic->HeightAboveEllipsoid < -10.0 && done == 1)
      switch (
          MAG_Warnings(3, CoordGeodetic->HeightAboveEllipsoid, MagneticModel)) {
      case 0:
        return 0;
      case 1:
        done = 0;
      ...
      if (!(MagneticDate->DecimalYear == a)) {
        if (!MAG_DateToYear(MagneticDate, Error_Message)) {
          ...
          printf("\nError encountered, please re-enter Date in MM/DD/YYYY or "
                 "MM DD YYYY format, or as a decimal year\n");
          fgets(buffer, 40, stdin);
          i = -1;
        } else
          done = 1;
...
for a failure. It copies an error message to the Error string in the event of a
failure.
 ...
 */
{
  ...
  for (i = 0; i <= n - 1; i++) /*tests for legal characters*/
  {
    if ((input[i] < '0' || input[i] > '9') &&
        (input[i] != ',' && input[i] != ' ' && input[i] != '-' &&
         input[i] != '\0' && input[i] != '\n')) {
      strcpy(Error, "\nError: Input contains an illegal character, legal "
                    "characters for Degree, Minute, Second format are:\n '0-9' "
                    "',' '-' '[space]' '[Enter]'\n");
      return FALSE;
    }
    if (input[i] == ',')
  ...
  if (j != 3) {
    strcpy(Error, "\nError: Not enough numbers used for Degrees, Minutes, "
                  "Seconds format\n or they were incorrectly formatted\n The "
                  "legal format is DD,MM,SS or DD MM SS\n");
    return FALSE;
  }
...
/*Validates a given longitude DMS string and returns 1 for a success and returns
0 for a failure. It copies an error message to the Error string in the event of
a failure.
 ...
 */
{
  ...
  if (j != 3) {
    strcpy(Error, "\nError: Not enough numbers read for Degrees, Minutes, "
                  "Seconds format\n or they were incorrectly formatted\n The "
                  "legal format is DD,MM,SS or DD MM SS\n");
    return FALSE;
  }
  sscanf(input, "%d, %d, %d", &degree, &minute,
         &second); /* tests for legal formatting and range */
  if (degree > 360 || degree < -180) {
    strcpy(Error, "\nError: Degree input is outside legal range\n The legal "
                  "range is from -180 to 360\n");
    return FALSE;
  }
  if (degree == 360 || degree == -180)
    max_minute = 0;
 ...
 */
{
  ...
  case 1: /* Horizontal Field strength low */
    printf("\nWarning: The Horizontal Field strength at this location is only "
           "%f\n",
           value);
    printf("	Compass readings have large uncertainties in areas where "
  ...
  case 2: /* Horizontal Field strength very low */
    printf("\nWarning: The Horizontal Field strength at this location is only "
           "%f\n",
           value);
    printf("	Compass readings have VERY LARGE uncertainties in areas "
...
void MAG_PrintSHDFFormat(char *filename,
  ...
  for (i = 0; i < epochs; i++) {
    ...
    fprintf(SHDF_file, "%%SHDF 16695 Definitive Geomagnetic Reference Field "
                       "Model Coefficient File\n");
    fprintf(SHDF_file, "%%ModelName: %s\n", (*MagneticModel)[i]->ModelName);
    fprintf(SHDF_file, "%%Publisher: International Association of Geomagnetism "
                       "and Aeronomy (IAGA), Working Group V-Mod\n");
...
MAGtype_GeoMagneticElements
MAG_GeoMagneticElementsScale(MAGtype_GeoMagneticElements Elements,
                             double factor) {
...
MAGtype_GeoMagneticElements
MAG_GeoMagneticElementsSubtract(MAGtype_GeoMagneticElements minuend,
...
/* Computes Spherical variables
   ...
   elements double lambda; ( longitude) double phig; ( geocentric latitude )
                             double r;  	  ( distance from the center of the
   ellipsoid)
 ...
 */
{
  ...
  if (Geoid->UseGeoid == 1) { /* Geoid correction required */
    ...
    CoordGeodetic->HeightAboveEllipsoid =
        CoordGeodetic->HeightAboveGeoid +
        DeltaHeight / 1000; /*  Input and output should be kilometers,
However MAG_GetGeoidHeight returns Geoid height in meters - Hence division by
1000 */
